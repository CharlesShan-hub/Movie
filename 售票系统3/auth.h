...
bool regist_get_id(int type)
{
    ...
    while (true)
    {
        ...
        {
            if ((id_str[i] <= 'Z' && id_str[i] >= 'A') || (id_str[i] <= 'z' && id_str[i] >= 'a'))
                letter_flag = true;
            if (id_str[i] <= '9' && id_str[i] >= '0')
                number_flag = true;
...
bool regist_get_tel()
{
    while (true)
    {
        ...
        if (!strcmp(U_quanju.Tele, "EOF"))
            return true; // EOF
        if (strlen(U_quanju.Tele) != 11)
        { // 保证电话11位
...
void regist()
{
    ...
    while (true)
    {
        ...
        case 1: // 注册管理员
            ...
            if (!strcmp(A_quanju.Name, "EOF"))
                continue;
            // 获取所属影院名称
            printf("请输入您的影院:\n");
            fflush(stdin);
            scanf("%s", A_quanju.Cinema);
            if (!strcmp(A_quanju.Cinema, "EOF"))
                continue;
            // 获取用户密码
        ...
        case 2: // 注册用户
            ...
            if (!strcmp(U_quanju.Name, "EOF"))
                continue;
            // 获取用户性别
            printf("请输入您的性别:\n");
            fflush(stdin);
            scanf("%s", U_quanju.Sex);
            if (!strcmp(U_quanju.Sex, "EOF"))
                continue;
            // 获取用户电话
            if (regist_get_tel())
                continue;
            // 获取用户密码
            if (regist_get_code(2))
                continue;
            // 获取用户邮箱
            if (regist_get_mail(2))
                continue;
            // 信息登记
