...
void A_change()
{
  ...
  while (true)
  {
    ...
    case 1:                             // 修改姓名
      ...
      fflush(stdin);
      scanf("%s", temp.Name);
      if (!strcmp(temp.Name, "EOF"))
        continue;
      strcpy(A_quanju.Name, temp.Name);
      break;
    case 2: // 修改邮箱
      if (regist_get_mail(1))
        continue;
      break;
    case 3: // 修改密码
      if (regist_get_code(1))
        continue;
      break;
    default:
      return;
...
void A_screen_add()
{
  ...
  char temp_str2_room[8] = "";
  A_screen_add_itoa(info.room, temp_str2_room);
...
bool A_session_add_get_room(int &room)
{
  ...
  while (true)
  {
    ...
    fflush(stdin);
    scanf("%s", temp_str);
    if (!strcmp(temp_str, "EOF"))
      return false;
    if (judge_num(temp_str))
    {
      printf("请输入数字!\n");
      continue;
    }
...
bool A_session_add_get_time_year(int &year)
{
  ...
  while (true)
  {
    ...
    if (atoi(temp_str) > 2100 || atoi(temp_str) < 2023)
    {
      printf("请输入2023到2100的一个数!\n");
      continue;
    }
...
bool A_session_add_get_time_month(int &month)
{
  ...
  while (true)
  {
    ...
    if (atoi(temp_str) > 12 || atoi(temp_str) < 1)
    {
      printf("请输入1到12的一个数!\n");
      continue;
    }
...
bool A_session_add_get_time_hour(int &hour)
{
  ...
  while (true)
  {
    ...
    if (atoi(temp_str) > 23 || atoi(temp_str) < 0)
    {
      printf("请输入0到23的一个数!\n");
      continue;
    }
...
bool A_session_add_get_time_minute(int &minute)
{
  ...
  while (true)
  {
    ...
    if (atoi(temp_str) > 59 || atoi(temp_str) < 0)
    {
      printf("请输入0到59的一个数!\n");
      continue;
    }
...
void A_session_add()
{
  ...
  while (true)
  {
    ...
    // 获取影片名
    if (!A_session_add_get_name(info.moviename))
      break;
    // 获取放映厅号
    if (!A_session_add_get_room(info.room))
      break;
    // 获取语言种类
    if (!A_session_add_get_len(info.language))
      break;
    // 获取票价
    if (!A_session_add_get_price(info.fee))
      break;
    // 获取影片时长
    if (!A_session_add_get_duration(info.duration))
      break;
    // 获取影片开始时间 - 年
    if (!A_session_add_get_time_year(info.year))
      break;
    // 获取影片开始时间 - 月
    if (!A_session_add_get_time_month(info.month))
      break;
    // 获取影片开始时间 - 日
    if (!A_session_add_get_time_day(info.day, info.month, info.year))
      break;
    // 获取影片开始时间 - 时
    if (!A_session_add_get_time_hour(info.hour))
      break;
    // 获取影片开始时间 - 分
    if (!A_session_add_get_time_minute(info.minute))
      break;
    // 验证
...
bool A_session_add_print(MovieMessage info)
{
  ...
  S_Message = fopen("ScreenMessage.txt", "r");
  while (fscanf(S_Message, "%s  %d  %s", SM.cinema, &SM.room, SM.type) != EOF)
    if (strcmp(SM.cinema, A_quanju.Cinema) == 0 && SM.room == info.room)
      break;
  fclose(S_Message);
  // 场次序号   count_
  ...
  while (fscanf(M_Message, "%s  %s  %s  %d  %d  %d  %d  %d  %d  %d  %s  %s  %d  %d  %f  %f",
                ...
                &MM.day, &MM.hour, &MM.minute, &MM.duration, MM.type, MM.language,
                &MM.restticket, &MM.totalticket, &MM.ratio, &MM.fee) != EOF)
    if (count_ < atoi(MM.movieID))
      count_ = atoi(MM.movieID);
  count_++;
  fclose(M_Message);
  // 余票数与总票数  num
...
bool A_session_add_valid(MovieMessage info)
{
                ...
                &MM.restticket, &MM.totalticket, &MM.ratio, &MM.fee) != EOF)
  {
    ...
    if (start + 20 * 60 >= end_temp || end + 20 * 60 <= end_temp)
      continue;
    flag = false;
  }
  fclose(file);
